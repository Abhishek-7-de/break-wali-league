...
function getRecaptcha(containerId = 'recaptcha-container') {
  ...
  recaptchaRef = new RecaptchaVerifier(auth, containerId, { size: 'normal' });
  return recaptchaRef;
}
...
async function saveGameResult({ uid, mode, outcome, config }) {
  ...
  const user = normalizeDailyUser(snap.data());
  const multiplier = boostWindow(config) ? config.matchBoost.multiplier : 1;
...
async function claimDailyBonus({ uid, bonusPoints = 5 }) {
  const ref = doc(db, 'users', uid);
  const snap = await getDoc(ref);
  if (!snap.exists()) throw new Error('User profile not found');
  ...
  const key = todayKey();
  if ((user.lastBonusClaimDate || '') === key) {
    return { alreadyClaimed: true, user };
  }

  await updateDoc(ref, {
    totalPoints: (user.totalPoints || 0) + bonusPoints,
    lastBonusClaimDate: key,
    updatedAt: serverTimestamp()
  });
